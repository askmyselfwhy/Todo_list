...
const checkTask = (list, taskId) => {
	return [ ...list.tasks ].map((item) => (item.id === taskId ? { ...item, isChecked: !item.isChecked } : item));
};
const deleteTask = (list, taskId) => {
	return [ ...list.tasks ].filter((item) => (item.id === taskId ? false : true));
};
const deleteAllCheckedTasks = (list) => {
	return [ ...list.tasks ].filter((item) => (!item.isChecked ? true : false));
};
